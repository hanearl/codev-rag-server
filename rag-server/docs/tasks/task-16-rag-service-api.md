# Task 16-2: RAG 서비스 통합 API 구현

## 📋 작업 개요
Task 13-15에서 구현된 LLM 체인 및 RAG 서비스 기능들에 대한 통합 API 엔드포인트를 구현합니다. 프롬프트 관리, LLM 체인, HybridRAG 서비스를 위한 RESTful API와 WebSocket 스트리밍을 제공합니다.

## 🎯 작업 목표
- RAG 관련 통합 API 엔드포인트 구현
- 스트리밍 질의응답 API 구현
- 프롬프트 관리 API 구현
- WebSocket을 통한 실시간 스트리밍 지원

## 🔗 의존성
- **선행 Task**: Task 13 (LangChain Prompt), Task 14 (LangChain LLMChain), Task 15 (HybridRAG 서비스 구현), Task 16-1 (인덱싱 및 검색 API)
- **후속 Task**: Task 17 (기존 개별 features 모듈들을 통합 API로 대체)
