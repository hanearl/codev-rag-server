...
class RAGServerBM25Adapter(RAGSystemInterface):
    """RAG 서버 BM25 검색 전용 어댑터"""
    ...
    async def retrieve(self, query: str, top_k: int = 10) -> List[RetrievalResult]:
        ...
        try:
            ...
            response = await self.client.post(
                self.config.endpoints.bm25_search,
                json=search_request
            )
            ...
            if not data.get("success", False):
                error_msg = data.get("error", "BM25 검색 실패")
                logger.error(f"BM25 검색 실패: {error_msg}")
                return []
            ...
            logger.info(f"BM25 검색 완료: {len(results)}개 결과, {data.get('search_time_ms', 0)}ms")
            return results
            
        except httpx.HTTPError as e:
            logger.error(f"BM25 검색 HTTP 오류: {e}")
            raise
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"BM25 검색 헬스체크 실패: {e}")
            return False
        ...
        try:
            ...
            info.update({
                "bm25_search_info": {
                    ...
                    "bm25_components": health_data.get("components", {}).get("bm25_index", False),
                    "features": {
                        "bm25_search": True,
                        "keyword_search": True,
                        "term_frequency": True,
                        "inverse_document_frequency": True,
                        "statistical_ranking": True
                    }
                }
            })
            ...
            logger.warning(f"BM25 검색 시스템 정보 조회 실패: {e}")
    ...
    def __repr__(self):
        return f"<RAGServerBM25Adapter(name='{self.config.name}', index='{self.config.index_name}')>" 